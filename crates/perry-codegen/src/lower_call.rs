...
enum NativeArgKind {
    ...
    JsvalI64,
    /// 32-bit integer. Converts from NaN-boxed f64 via fptosi.
    I32,
    /// Boolean (passed as i32 0/1). Converts from NaN-boxed f64.
    Bool,
}
...
const NATIVE_MODULE_TABLE: &[NativeModSig] = &[
    ...
    NativeModSig { module: "perry/container", has_receiver: false, method: "remove",
        class_filter: None,
        runtime: "js_container_remove", args: &[NA_STR, NA_BOOL], ret: NR_PTR },
    NativeModSig { module: "perry/container", has_receiver: false, method: "list",
        class_filter: None,
        runtime: "js_container_list", args: &[NA_BOOL], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/container", has_receiver: false, method: "removeImage",
        class_filter: None,
        runtime: "js_container_removeImage", args: &[NA_STR, NA_BOOL], ret: NR_PTR },
...
fn lower_native_module_dispatch(
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        match kind {
            ...
            NativeArgKind::I32 => {
                let blk = ctx.block();
                let val = blk.fptosi(DOUBLE, &lowered, I32);
                llvm_args.push((I32, val));
                arg_types.push(I32);
            }
            NativeArgKind::Bool => {
                // Perry booleans are Tagged NaN values (TAG_TRUE/TAG_FALSE).
                // unbox_to_i64 followed by comparison is best.
                let blk = ctx.block();
                let bits = unbox_to_i64(blk, &lowered);
                let is_true = blk.icmp_ne(I64, &bits, "0");
                let i32_val = blk.select(crate::types::I1, &is_true, I32, "1", "0");
                llvm_args.push((I32, i32_val));
                arg_types.push(I32);
            }
        }
    }
    ...
    for i in args.len()..sig.args.len() {
        match sig.args[i] {
            ...
            NativeArgKind::I32 | NativeArgKind::Bool => {
