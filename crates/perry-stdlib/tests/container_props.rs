...
proptest! {
    ...
    fn prop_depends_on_entry_service_names(
        ...
        // Set deduplication handled by the spec internally
        let expected_unique_count = names.iter().collect::<std::collections::HashSet<_>>().len();
        prop_assert_eq!(list_names.len(), names.len());
        prop_assert_eq!(map_names.len(), expected_unique_count);
...
proptest! {
    ...
    fn prop_handle_registry_type_safety(
        ...
        let taken: Vec<ContainerInfo> = taken.unwrap();
