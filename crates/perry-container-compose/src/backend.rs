...
pub trait CliProtocol: Send + Sync {
    ...
    fn parse_list_output(&self, stdout: &str) -> Vec<ContainerInfo> {
        ...
        stdout.lines().filter_map(|line| {
            serde_json::from_str::<Value>(line).ok().and_then(|v| parse_container_info_from_json(&v).ok())
        }).collect()
    }
    ...
    fn parse_list_images_output(&self, stdout: &str) -> Vec<ImageInfo> {
        ...
        stdout.lines().filter_map(|line| {
            serde_json::from_str::<Value>(line).ok().and_then(|v| parse_image_info_from_json(&v).ok())
        }).collect()
    }
...
impl CliProtocol for AppleContainerProtocol {
    ...
    fn parse_inspect_output(&self, _id: &str, stdout: &str) -> Option<ContainerInfo> {
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
            let val = if v.is_array() { v.as_array()?.first()? } else { &v };
            let config = val.get("configuration")?;
            let id = config.get("id")?.as_str()?.to_string();
            let image = config.get("image")?.get("reference")?.as_str()?.to_string();
            let status = val.get("status")?.as_str()?.to_string();
            return Some(ContainerInfo { id, name: "".into(), image, status, ports: vec![], created: "".into() });
        }
        None
    }
    fn parse_list_output(&self, stdout: &str) -> Vec<ContainerInfo> {
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
            if let Some(arr) = v.as_array() {
                return arr.iter().filter_map(|val| {
                    let config = val.get("configuration")?;
                    let id = config.get("id")?.as_str()?.to_string();
                    let image = config.get("image")?.get("reference")?.as_str()?.to_string();
                    let status = val.get("status")?.as_str()?.to_string();
                    Some(ContainerInfo { id, name: "".into(), image, status, ports: vec![], created: "".into() })
                }).collect();
            }
        }
        vec![]
...
impl<P: CliProtocol> CliBackend<P> {
    ...
    async fn exec_raw(&self, subcommand_args: Vec<String>) -> Result<std::process::Output> {
        ...
        final_args.extend(subcommand_args);
...
pub async fn detect_backend() -> std::result::Result<Box<dyn ContainerBackend + Send + Sync>, Vec<BackendProbeResult>> {
    ...
    for &name in candidates {
        match timeout(Duration::from_secs(2), probe_candidate_backend(name)).await {
            Ok(Ok(backend)) => return Ok(backend),
            Ok(Err(reason)) => results.push(BackendProbeResult { name: name.into(), available: false, reason }),
            Err(_) => results.push(BackendProbeResult { name: name.into(), available: false, reason: "probe timed out".into() }),
        }
    }
    Err(results)
}

async fn probe_candidate_backend(name: &str) -> std::result::Result<Box<dyn ContainerBackend + Send + Sync>, String> {
    match name {
        "apple/container" => {
        ...
        "colima" => {
            let bin = which::which("colima").map_err(|_| "colima binary not found")?;
...
async fn probe_candidate(name: &str) -> std::result::Result<Box<dyn ContainerBackend + Send + Sync>, String> {
    probe_candidate_backend(name).await
}
...
async fn check_orbstack_available() -> std::result::Result<(), String> {
    let socket = home::home_dir().ok_or("no home dir")?.join(".orbstack/run/docker.sock");
...
async fn check_rancher_desktop_available() -> std::result::Result<(), String> {
    let socket = home::home_dir().ok_or("no home dir")?.join(".rd/run/containerd-shim.sock");
